...
from plancklens.sims import pico2021_sims, cmbs, phas, maps, utils as maps_utils

directory =   "PICO/nilc_nomask_1nlev" 
dset = directory.split('/')[0]

os.environ['PLENS'] = '/global/cscratch1/sd/sebibel/plens/'
assert 'PLENS' in os.environ.keys(), 'Set env. variable PLENS to a writeable folder'
TEMP =  os.path.join(os.environ['PLENS'], 'temp', directory)
cls_path = os.path.join(os.path.dirname(os.path.abspath(plancklens.__file__)), 'data', 'cls')

sims_raw  = pico2021_sims.nilc_90b91_nside2048()

simp = sims_raw.get_noise_sim_pmap(1)
CLBBN_M = hp.anafast([np.zeros_like(simp[0]), *simp], lmax=2048, pol=True)
...
lmax_general = 2048
transf = sims_raw.get_tf(lmax_general, nside)

nlev_p = 1 # np.sqrt(CLBBN_M)/transf * (60 * 180 / np.pi)
nlev_t = 1 # np.sqrt(CLBBN_M)/transf * (60 * 180 / np.pi) / np.sqrt(2)
...
# Masks
Tmaskpaths = ['/global/homes/s/sebibel/data/mask/nomask.fits.gz']
